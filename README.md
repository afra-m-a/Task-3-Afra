...
**ThreatScope** is a Python application designed for security analysts to quickly triage suspicious messages, calculate risk metrics, and assign automated incident response paths. The current GUI runs a self-contained, rule-based heuristic scoring loop, while supplementary standalone reference modules are included to demonstrate advanced threat parsing architecture.

---

## 🚀 Key Features

- **Live Payload Analysis** – Drop raw email body elements and headers into a verification zone for immediate pattern parsing.
...
- **Dynamic Heuristic Scoring Matrix** – Computes a consolidated metric scale ($0 \le \text{Score} \le 100$) using multi-weighted threat indicators.
- **Deterministic Action Routing** – Maps alert values directly to standardized enterprise response playbooks:
...
> **Note:** The running application executes its core engine out of `gui.py` and `main.py`. The `parser.py` and `triage.py` source files are bundled purely as modular design blueprints for enterprise reference.
...
Add target search expressions or specific infrastructure patterns inside `analyze_payload()` in `gui.py`:
...
Tune defensive playbook routing profiles by shifting metric conditions inside `gui.py`:
